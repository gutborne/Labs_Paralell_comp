...
int main(int argc, char **argv){
    ...
    printf("process %d: Teste de deadlock.", rank);
    if(rank == 0){
        dest = source = 1;
        //Process 0 and 1 receiving at the same time and generating deadlock
        //MPI_Recv(&my_buffer, 1, MPI_CHAR, source, tag, MPI_COMM_WORLD, &status);
        //MPI_Send(&message, 1, MPI_CHAR, dest, tag, MPI_COMM_WORLD);
        
        //Process 0 and 1 sending at the same time but it doesnt generate deadlock
        //MPI_Send(&message, 1, MPI_CHAR, dest, tag, MPI_COMM_WORLD);
        //MPI_Recv(&my_buffer, 1, MPI_CHAR, source, tag, MPI_COMM_WORLD, &status);
    }else if(rank == 1){
        dest = source = 0;
